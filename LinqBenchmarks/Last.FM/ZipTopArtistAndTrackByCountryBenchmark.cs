...
namespace LinqBenchmarks.Last.FM
{
    public class ZipTopArtistAndTrackByCountryBenchmark : BenchmarkBase
    {
        ...
        public int ForeachLoop()
        {
            ...
            {
                var tuple = Tuple.Create(artistsByCountry[i].Item1, artistsByCountry[i].Item2.First(), tracksByCountry[i].Item2.First());
        ...
        public int Linq()
        {
            ...
            var query = artistsByCountry
                .Zip(tracksByCountry, (l, r) => Tuple.Create(l.Item1, l.Item2.First(), r.Item2.First()))
        ...
        public int Hyperlinq()
        {
            ...
            var artistsByCountry = countries.data.AsValueEnumerable().Where(country => isNonEnglishSpeaking(country))
                .Where(country => artists.data.ContainsKey(country.name) && !artists.data[country.name].IsEmpty())
                .Select(country => Tuple.Create(country, artists.data[country.name].AsValueEnumerable()));
            var tracksByCountry = countries.data.AsValueEnumerable().Where(country => isNonEnglishSpeaking(country))
                .Where(country => tracks.data.ContainsKey(country.name) && !tracks.data[country.name].IsEmpty())
